# BHSD Multi-class DPT-style Segmentation: EDH, IPH, IVA, SAH, SDH (Fixed Version)
...
class BHSDDPTSegmentationDataset(Dataset):
...
class DPTMultiClassSegmentationHead(nn.Module):
    """
    DPT风格的多类分割头，仿照原始DPT设计
    """
    def __init__(
        self, 
        num_classes,  # 6类: 背景 + 5种血肿
        in_channels,  # DINOv3的特征维度 (768)
        features=256,  # 统一特征维度
        out_channels=[96, 192, 384, 768],  # 不同层的投影通道数
    ):
        super(DPTMultiClassSegmentationHead, self).__init__()
        ...
        for i, out_ch in enumerate(out_channels):
            refinement_layers.append(_make_scratch_layer(out_ch, features))
        self.refinement_layers = nn.ModuleList(refinement_layers)
        
        # 最终输出卷积层 - 多类分割
        self.output_conv = nn.Sequential(
            nn.Conv2d(features * len(out_channels), features, kernel_size=3, stride=1, padding=1),
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(features // 4, num_classes, kernel_size=1, stride=1, padding=0)
        )
        
        print(f"DPT Multi-class Head: {len(out_channels)} layers -> {out_channels} -> {[features]*len(out_channels)} -> {num_classes} classes")
    ...
    def forward(self, out_features):
        ...
        for i, x in enumerate(out_features):
            ...
            x_refined = self.refinement_layers[i](x_proj)  # [B, features, H, W]
            refined_features.append(x_refined)
        
        # 获取目标尺寸 (使用第一层的尺寸作为目标)
        target_hw = refined_features[0].shape[-2:]
        
        # 上采样到统一尺寸
        upsampled_features = []
        for features in refined_features:
            if features.shape[-2:] != target_hw:
                features = F.interpolate(features, size=target_hw, mode="bilinear", align_corners=True)
            upsampled_features.append(features)
        
        # 特征融合
        fused = torch.cat(upsampled_features, dim=1)  # [B, features*num_layers, H, W]
...
class DINOv3DPTMultiClassSegmentationModel(nn.Module):
    def __init__(self, dinov3_model_path, layer_indices=[2, 5, 8, 11], num_classes=6, features=256):
        super().__init__()
        ...
        # DPT风格的多类分割头
        self.segmentation_head = DPTMultiClassSegmentationHead(
...
def create_output_dir(model_path, layer_config):
    ...
    output_dir = f"output_bhsd_dpt_multiclass_seg/{model_name}_layers_{layer_str}_{timestamp}"
...
def plot_training_curves(train_history, output_dir, layer_indices):
    ...
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(f'DPT Loss (Layers: {layer_indices})')
    plt.legend()
    plt.grid(True)
    ...
    for i, class_name in enumerate(class_names):
        ...
        plt.title(f'DPT {class_name} Dice Score')
        plt.legend()
        plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'training_curves_dpt_multiclass.png'), dpi=300, bbox_inches='tight')
    plt.close()
...
def train_model_with_layer_config(layer_indices):
    ...
    # 创建数据集 - 使用文件列表
    train_dataset = BHSDDPTSegmentationDataset(images_dir, labels_dir, processor, 
                                             file_list=train_files, is_train=True)
    test_dataset = BHSDDPTSegmentationDataset(images_dir, labels_dir, processor, 
    ...
    # 创建模型 - 使用新的DPT风格多类模型
    model = DINOv3DPTMultiClassSegmentationModel(
        dinov3_model_path=model_path, 
        layer_indices=layer_indices, 
        num_classes=6,  # 6类：背景 + 5种血肿
    ...
    # 保存训练配置
    config_info = f"""BHSD Multi-class DPT Segmentation Training Configuration:
Model: {os.path.basename(model_path)}
Architecture: DINOv3 + DPT Multi-class Head
Layer Indices: {layer_indices}
Task: Multi-class DPT Segmentation (Background + EDH + IPH + IVA + SAH + SDH)
Features: 256
Device: {device}
Epochs: {num_epochs}
Batch Size: 8
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (images, masks) in enumerate(pbar):
            ...
            if not debug_printed:
                print(f"\nDebugging DPT multi-class model shapes:")
                ...
                print("DPT multi-class debug completed. Continuing training...")
                debug_printed = True
            else:
                outputs = model(images)
        ...
        print("Per-class DPT Dice scores:")
        for i, class_name in enumerate(class_names):
        ...
        if mean_val_dice > best_dice:
            ...
            print(f'New best DPT multi-class model saved with Mean Dice: {best_dice:.4f}')
        else:
            patience_counter += 1
...
def evaluate_model_multilayer_multiclass(output_dir, model_path, layer_indices, test_files):
    ...
    print(f"Starting DPT multi-class segmentation evaluation on {len(test_dataset)} test samples...")
    ...
    with torch.no_grad():
        for idx, (images, masks) in enumerate(tqdm(test_loader, desc='Evaluating')):
            ...
            if idx < 5:
                ...
                axes[2].set_title('DPT Prediction')
                axes[2].axis('off')
                ...
                fig.suptitle(f'DPT Sample {idx+1} (Layers: {layer_indices}): {class_names}', fontsize=14)
                
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, 'visualizations', f'dpt_multiclass_result_{idx:03d}.png'), 
    ...
    if len(all_predictions) > 0 and len(all_targets) > 0:
        ...
        plt.title(f'DPT Multi-class Segmentation Confusion Matrix\n(Layers: {layer_indices})')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'dpt_confusion_matrix.png'), dpi=300, bbox_inches='tight')
        plt.close()
    else:
        cm = np.zeros((6, 6))
    
    # 生成结果报告
    results_text = f"""BHSD DPT Multi-class Segmentation Evaluation Results:
Model: {os.path.basename(model_path)}
Architecture: DINOv3 + DPT Multi-class Head
Layer Indices: {layer_indices}
...
Notes:
- Architecture uses DPT-style feature pyramid with layer-wise refinement for multi-class segmentation
...
"""
    ...
    # 保存结果
    with open(os.path.join(output_dir, 'dpt_evaluation_results.txt'), 'w') as f:
        f.write(results_text)
    
    # 保存详细结果到JSON
    results_dict = {
        'architecture': 'DINOv3_DPT_MultiClass',
        'layer_indices': layer_indices,
    ...
    with open(os.path.join(output_dir, 'dpt_evaluation_results.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    print("Starting BHSD DPT-style multi-class multi-layer segmentation experiments...")
    
    for i, layer_indices in enumerate(layer_configs):
        print(f"\n{'='*60}")
        print(f"BHSD DPT Experiment {i+1}/{len(layer_configs)}: Layers {layer_indices}")
        print(f"{'='*60}")
        ...
        try:
            ...
            print(f"BHSD DPT Experiment {i+1} completed successfully!")
            
        except Exception as e:
            print(f"BHSD DPT Experiment {i+1} failed with error: {e}")
            continue
    
    # 保存实验总结
    print(f"\n{'='*60}")
    print("BHSD DPT MULTI-CLASS EXPERIMENT SUMMARY")
    print(f"{'='*60}")
    
    summary_text = "BHSD DPT Multi-Class Multi-Layer DINOv3 Segmentation Experiments Summary\n"
    summary_text += "="*60 + "\n\n"
    
    for i, result in enumerate(results_summary):
        summary_text += f"BHSD DPT Experiment {i+1}:\n"
        ...
        print(f"DPT Layers {result['layers']}: Dice={result['final_dice']:.4f}, IoU={result['final_iou']:.4f}")
    
    # 保存总结到文件
    with open('bhsd_dpt_multiclass_experiments_summary.txt', 'w') as f:
        f.write(summary_text)
    
    print(f"\nAll BHSD DPT multi-class experiments completed!")
    print(f"Summary saved to 'bhsd_dpt_multiclass_experiments_summary.txt'")