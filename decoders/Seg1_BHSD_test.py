# BHSD Multi-class Segmentation: EDH, IPH, IVA, SAH, SDH (Fixed Version)
import torch
...
class BHSDSegmentationDataset(Dataset):
...
class DINOv3MultiClassSegmentationModel(nn.Module):
    def __init__(self, dinov3_model_path, num_classes=6):  # 6类: 背景 + 5种血肿
        super().__init__()
...
def plot_training_curves(train_history, output_dir):
    ...
    for i, class_name in enumerate(class_names):
        ...
        plt.title(f'{class_name} Dice Score')
        plt.legend()
        plt.grid(True)
...
def train_model():
    ...
    # 创建数据集 - 使用文件列表
    train_dataset = BHSDSegmentationDataset(images_dir, labels_dir, processor, 
                                          file_list=train_files, is_train=True)
    test_dataset = BHSDSegmentationDataset(images_dir, labels_dir, processor, 
    ...
    # 创建模型
    model = DINOv3MultiClassSegmentationModel(model_path, num_classes=6).to(device)
    ...
    # 保存配置
    config_info = f"""BHSD Multi-class Segmentation Training Configuration:
Model: {os.path.basename(model_path)}
Task: Multi-class Segmentation (Background + EDH + IPH + IVA + SAH + SDH)
Device: {device}
Epochs: {num_epochs}
Batch Size: 8
Learning Rate: 1e-4
...
"""
    ...
    for epoch in range(num_epochs):
        ...
        if mean_val_dice > best_dice:
            ...
            print(f'New best model saved with Mean Dice: {best_dice:.4f}')
        else:
            patience_counter += 1
    ...
    return output_dir, test_files

def evaluate_model(output_dir, model_path, test_files):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    ...
    print(f"Starting multi-class segmentation evaluation on {len(test_dataset)} test samples...")
    ...
    with torch.no_grad():
        for idx, (images, masks) in enumerate(tqdm(test_loader, desc='Evaluating')):
            ...
            if idx < 5:
                ...
                fig.suptitle(f'Sample {idx+1}: Classes - {class_names}', fontsize=14)
                
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, 'visualizations', f'multiclass_result_{idx:03d}.png'), 
    ...
    if len(all_predictions) > 0 and len(all_targets) > 0:
        ...
        plt.title('Multi-class Segmentation Confusion Matrix')
        ...
        sns.heatmap(cm_normalized, annot=True, fmt='.3f', cmap='Blues', 
                    xticklabels=class_names, yticklabels=class_names)
        plt.title('Normalized Multi-class Segmentation Confusion Matrix')
        plt.ylabel('True Label')
        plt.xlabel('Predicted Label')
        plt.tight_layout()
        ...
        cm = np.zeros((6, 6))
        cm_normalized = np.zeros((6, 6))
    
    # 生成结果报告
    results_text = f"""BHSD Multi-class Segmentation Evaluation Results:
Model: {os.path.basename(model_path)}
...
if __name__ == "__main__":
    print("Starting BHSD multi-class segmentation training...")
    output_dir, test_files = train_model()
    
    print(f"\nStarting evaluation...")
    model_path = "/path/to/dinov3-vitb16-pretrain-lvd1689m"
    evaluate_model(output_dir, model_path, test_files)
